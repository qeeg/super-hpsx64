...
void Cpu::Run ()
{
	...
	*/
	...
	if ( Status.Value )
	{
		...
		{
			//if ( CPR0.Status.IEc )
			if ( ( CPR0.Status.IE && CPR0.Status.EIE ) && ( !CPR0.Status.EXL ) && ( !CPR0.Status.ERL ) )
			{
...
u32 Cpu::Read_MFC0 ( u32 Register )
{
	...
	switch ( Register )
	{
		case 9:
			...
			debug << ";Count" << ";Before=" << CPR0.Regs [ 9 ] << ";Writing=" << hex << Value;
#endif

			CPR0.Regs [ 9 ] = (u32) CycleCount;
			return (u32) CycleCount;
			break;
...
void Cpu::ProcessAsynchronousInterrupt ()
{
	...
	debug << "\r\nBranch Delay Instruction: " << R5900::Instruction::Print::PrintInstruction ( DelaySlot1.Instruction.Value ).c_str () << "  " << hex << DelaySlot1.Instruction.Value;
	debug << "\r\nR29=" << _CPU->GPR [ 29 ].uw0 << " R31=" << _CPU->GPR [ 31 ].uw0 << " 0x3201d0=" << Bus->MainMemory.b32 [ 0x3201d0 >> 2 ];
#endif
...
static void Cpu::DebugWindow_Enable ()
{
	...
	static const char* DebugWindow_Caption = "R5900 Debug Window";
	...
	if ( !DebugWindow_Enabled )
	{
		...
		GPR_ValueList->AddVariable ( "PCount", & GPU::_GPU->Primitive_Count );
		...
		*/
		...
		DisAsm_Window->Create ( DebugWindow, DisAsm_X, DisAsm_Y, DisAsm_Width, DisAsm_Height, R5900::Instruction::Print::PrintInstruction );
