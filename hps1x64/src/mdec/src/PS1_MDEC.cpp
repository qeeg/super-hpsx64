...
static u32 MDEC::Read ( u32 Address )
{
...
static void MDEC::Write ( u32 Address, u32 Data, u32 Mask )
{
...
// dma 0 - mdec in
bool MDEC::DMA_ReadyForWrite ()
{
	// for now, I'll say it's always ready for write
	// *** testing *** //
	
	// ready for write if the command is 0x2, 0x3, but not 0x1
	
	switch ( mess_mdec.n_0_command >> 29 )
	{
		case 0x1:
			return 0;
			break;
	...
	return mess_mdec.n_readyfordata_in;
...
int MDEC::DMA_Read ( u32 Address, u32 Size, u32* RAM )
{
	...
	//}
	debug << "; Size=" << dec << Size << "; n_readindex_input=" << (u32)( mess_mdec.n_readindex_input );
#endif

	// *** will use mame/mess device for now ***
	return mess_mdec.dma_read ( Address, Size, RAM/*, Size*/ );
}


void MDEC::DMA_Write ( u32 Address, u32 Size, u32* RAM )
{
	...
	// *** will use mame/mess device for now ***
	mess_mdec.dma_write ( Address, Size, RAM );
}
...
void psxmdec_device::mdec_makergb15( UINT32 n_address, INT32 n_r, INT32 n_g, INT32 n_b, INT32 *p_n_y, UINT16 n_stp, u32 XorValue )
{
...
void psxmdec_device::mdec_makergb24( UINT32 n_address, INT32 n_r, INT32 n_g, INT32 n_b, INT32 *p_n_y, UINT32 n_stp, u32 XorValue )
{
...
void psxmdec_device::dma_write( UINT32 n_address, INT32 n_size, u32* RAM )
{
	//psx_state *p_psx = machine().driver_data<psx_state>();
	// fix ram pointer
	//UINT32 *p_n_psxram = p_psx->m_p_n_psxram;
	UINT32 *p_n_psxram = RAM;
	//int n_index;
...
int psxmdec_device::dma_read( UINT32 n_address, INT32 n_size, u32* RAM )
{
	//psx_state *p_psx = machine().driver_data<psx_state>();
