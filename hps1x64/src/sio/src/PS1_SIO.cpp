...
if ( ( MemoryCard_State >> 1 ) == 1 )
{
	...
	debug << " (MEMCARDSETSECTOR) PacketSize=" << dec << PacketSize << " ";
					...
					// start memory card index at zero
					MemoryCardIndex = 0;
...
if ( ( MemoryCard_State >> 1 ) == 1 )
{
	...
	debug << " (MEMCARDWRITE) PacketSize=" << dec << PacketSize << " ";
					...
					// repeat the input starting at index 3
					if ( ( MemoryCard_State >> 1 ) > 2 )
					{
...
void SIO::Command_0x42 ( bool ForceAnalogOutput )
{
	...
	switch ( CTRL0 & 0x2002 )
	{
		case 0x2:
			...
			if ( Joy.joyinfo.dwPOV > 0 && Joy.joyinfo.dwPOV < 18000 )
			{
				// right arrow is down on joystick
				Output_Buf [ 3 ] ^= 0x20;
			}
			
			if ( Joy.joyinfo.dwPOV > 9000 && Joy.joyinfo.dwPOV < 27000 )
			{
				// down arrow is down on joystick
				Output_Buf [ 3 ] ^= 0x40;
			}
			
			if ( Joy.joyinfo.dwPOV > 18000 && Joy.joyinfo.dwPOV <= 36000 )
			{
				// left arrow is down on joystick
				Output_Buf [ 3 ] ^= 0x80;
			}
			
			if ( ( Joy.joyinfo.dwPOV < 9000 || Joy.joyinfo.dwPOV > 27000 ) && Joy.joyinfo.dwPOV <= 36000 && Joy.joyinfo.dwPOV >= 0 )
			{
				// up arrow is down on joystick
				Output_Buf [ 3 ] ^= 0x10;
			}
			
			if ( Joy.joyinfo.dwButtons & Key_Start [ 0 ] )
			{
				// start key //
				Output_Buf [ 3 ] ^= 0x08;
			}
			
			if ( Joy.joyinfo.dwButtons & Key_Select [ 0 ] )
			{
				// select key //
				Output_Buf [ 3 ] ^= 0x01;
			}
			
			if ( Joy.joyinfo.dwButtons & Key_X [ 0 ] )
			{
				// x key //
				Output_Buf [ 4 ] ^= 0x40;
			}
			
			if ( Joy.joyinfo.dwButtons & Key_O [ 0 ] )
			{
				// o key //
				Output_Buf [ 4 ] ^= 0x20;
			}
			
			if ( Joy.joyinfo.dwButtons & Key_Square [ 0 ] )
			{
				// square key //
				Output_Buf [ 4 ] ^= 0x80;
			}
			
			if ( Joy.joyinfo.dwButtons & Key_Triangle [ 0 ] )
			{
				// triangle key //
				Output_Buf [ 4 ] ^= 0x10;
			}
			
			if ( Joy.joyinfo.dwButtons & Key_L1 [ 0 ] )
			{
				// L1 key //
				Output_Buf [ 4 ] ^= 0x4;
			}
			
			if ( Joy.joyinfo.dwButtons & Key_L2 [ 0 ] )
			{
				// L2 key //
				Output_Buf [ 4 ] ^= 0x1;
			}
			
			if ( Joy.joyinfo.dwButtons & Key_R1 [ 0 ] )
			{
				// R1 key //
				Output_Buf [ 4 ] ^= 0x8;
			}
			
			if ( Joy.joyinfo.dwButtons & Key_R2 [ 0 ] )
			{
				// R2 key //
				Output_Buf [ 4 ] ^= 0x2;
			}
			
			if ( Joy.joyinfo.dwButtons & Key_L3 [ 0 ] )
			{
				// L3 key //
				Output_Buf [ 3 ] ^= 0x2;
			}
			
			if ( Joy.joyinfo.dwButtons & Key_R3 [ 0 ] )
			{
				// R3 key //
				Output_Buf [ 3 ] ^= 0x4;
			}
			...
			if ( /*Joy.joyinfo.dwRpos*/ ((u32*)(&Joy.joyinfo.dwXpos)) [ RightAnalog_X [ 0 ] ] == 0x7fff )
			{
				Output_Buf [ 5 ] = 0x80;
			}
			else
			{
				Output_Buf [ 5 ] = ( /*Joy.joyinfo.dwRpos*/ ((u32*)(&Joy.joyinfo.dwXpos)) [ RightAnalog_X [ 0 ] ] >> 8 );
			}
			
			// right analog stick - y-axis
			if ( /*Joy.joyinfo.dwZpos*/ ((u32*)(&Joy.joyinfo.dwXpos)) [ RightAnalog_Y [ 0 ] ] == 0x7fff )
			{
				Output_Buf [ 6 ] = 0x80;
			}
			else
			{
				Output_Buf [ 6 ] = ( /*Joy.joyinfo.dwZpos*/ ((u32*)(&Joy.joyinfo.dwXpos)) [ RightAnalog_Y [ 0 ] ] >> 8 );
			}
			
			// left analog stick - x-axis
			if ( /*Joy.joyinfo.dwXpos*/ ((u32*)(&Joy.joyinfo.dwXpos)) [ LeftAnalog_X [ 0 ] ] == 0x7fff )
			{
				Output_Buf [ 7 ] = 0x80;
			}
			else
			{
				Output_Buf [ 7 ] = ( /*Joy.joyinfo.dwXpos*/ ((u32*)(&Joy.joyinfo.dwXpos)) [ LeftAnalog_X [ 0 ] ] >> 8 );
			}
			
			// left analog stick - y-axis
			if ( /*Joy.joyinfo.dwYpos*/ ((u32*)(&Joy.joyinfo.dwXpos)) [ LeftAnalog_Y [ 0 ] ] == 0x7fff )
			{
				Output_Buf [ 8 ] = 0x80;
			}
			else
			{
				Output_Buf [ 8 ] = ( /*Joy.joyinfo.dwYpos*/ ((u32*)(&Joy.joyinfo.dwXpos)) [ LeftAnalog_Y [ 0 ] ] >> 8 );
			}
...
static u32 SIO::Read ( u32 Address )
{
...
static void SIO::Write ( u32 Address, u32 Data, u32 Mask )
{
...
bool SIO::SIO2in_DMA_Ready ()
{
	return SIO2_InputReady;
}

bool SIO::SIO2out_DMA_Ready ()
{
	return SIO2_OutputReady;
}
...
void SIO::DMA_ReadBlock ( u8* Data, int ByteCount )
{
	...
	debug << "\r\nDMA_ReadBlock";
	debug << "; TransferSize=" << dec << ByteCount;
	debug << " OutputReadIndex=" << dec << OutputReadIndex;
	debug << " SIO2_OutputIndex=" << dec << SIO2_OutputIndex;
	debug << " PacketSize=" << dec << PacketSize;
	debug << "; Output=";
#endif
	...
	//for ( int i = 0; i < TransferSize; i++ )
	{
		...
		Data [ i ] = SIO2_OutputBuffer [ OutputReadIndex++ ];
...
#ifdef INLINE_DEBUG_SIO2_DMA_WRITE
	debug << hex << setw(2) << (u32) Output_Buf [ i ] << " ";
#endif
		}
		else
		{
			SIO2_OutputBuffer [ SIO2_OutputIndex++ ] = 0;
		}
	}
...
static void SIO::Create_MemoryCardFile ( char* FileName )
{
	CreateMcd ( FileName );
}
