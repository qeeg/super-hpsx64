...
static u32 SPU::Debug_ChannelEnable = 0xffffff;

static HWAVEOUT SPU::hWaveOut; /* device handle */
static WAVEFORMATEX SPU::wfx;
static WAVEHDR SPU::header;
static WAVEHDR SPU::header0;
static WAVEHDR SPU::header1;
...
static const char* SPU::RegisterNames [ 256 + 48 ] = { 
	...
	"r1dc0", "r1dc2", "r1dc4", "r1dc6", "r1dc8", "r1dca", "r1dcc", "r1dce", "r1dd0", "r1dd2", "r1dd4", "r1dd6", "r1dd8", "r1dda", "r1ddc", "r1dde",
	"r1de0", "r1de2", "r1de4", "r1de6", "r1de8", "r1dea", "r1dec", "r1dee", "r1df0", "r1df2", "r1df4", "r1df6", "r1df8", "r1dfa", "r1dfc", "r1dfe",
...
//#endif
	...
	debug << "; (after) VOL_ADSR_Value=" << hex << VOL_ADSR_Value [ Channel ] << "; Cycles=" << dec << Cycles [ Channel ] << "; Value=" << hex << ( ( ModeRate >> 8 ) & 0x7f ) << "; flags=" << ( ( ModeRate >> 15 ) << 1 );
					...
					/*
					if ( VOL_ADSR_Value [ Channel ] < 0x6000 )
					{
						VOL_ADSR_Value [ Channel ] += VOL_ATTACK_Constant [ Channel ];
					}
					else
					{
						VOL_ADSR_Value [ Channel ] += VOL_ATTACK_Constant75 [ Channel ];
					}
					*/
	...
	debug << "; Decay";
					...
					/*
					Temp = (u64) (VOL_ADSR_Value [ Channel ]);
					VOL_ADSR_Value [ Channel ] = (u32) ( ( Temp >> 15 ) * ( VOL_DECAY_Constant [ Channel ] >> 15 ) );
					//VOL_ADSR_Value [ Channel ] = ( ( VOL_ADSR_Value [ Channel ] >> 16 ) * VOL_DECAY_Constant [ Channel ] ) >> 15;
					*/
	...
	debug << "; (after) VOL_ADSR_Value=" << hex << VOL_ADSR_Value [ Channel ] << "; Cycles=" << dec << Cycles [ Channel ] << "; Value=" << hex << ( ( ModeRate >> 8 ) & 0x7f ) << "; flags=" << ( ( ModeRate >> 15 ) << 1 );
					...
					/*
					// check if increase or decrease (adsr1 bit 14)
					if ( ( ( Regs [ ( ADSR_1 >> 1 ) + ( Channel << 3 ) ] >> 14 ) & 3 ) != 3 )
					{
						/////////////////////////////////////////////
						...
						if ( VOL_ADSR_Value [ Channel ] < 0x6000 )
						{
							VOL_ADSR_Value [ Channel ] += VOL_SUSTAIN_Constant [ Channel ];
						}
						else
						{
							VOL_ADSR_Value [ Channel ] += VOL_SUSTAIN_Constant75 [ Channel ];
						}
					...
					else
					{
						...
						Temp = (u64) (VOL_ADSR_Value [ Channel ]);
						VOL_ADSR_Value [ Channel ] = (s64) ( ( Temp >> 15 ) * ( VOL_SUSTAIN_Constant [ Channel ] >> 15 ) );
						//VOL_ADSR_Value [ Channel ] = ( ( VOL_ADSR_Value [ Channel ] >> 16 ) * VOL_SUSTAIN_Constant [ Channel ] ) >> 15;
					}
					*/
	...
	debug << "; (after) VOL_ADSR_Value=" << hex << VOL_ADSR_Value [ Channel ] << "; Cycles=" << dec << Cycles [ Channel ] << "; Value=" << hex << ( ( ModeRate >> 8 ) & 0x7f ) << "; flags=" << ( ( ModeRate >> 15 ) << 1 );
					...
					/*
					if ( ! ( Regs [ ( ADSR_1 >> 1 ) + ( Channel << 3 ) ] & 0x20 ) )
					{
						////////////////////////////
						// linear decrease
						VOL_ADSR_Value [ Channel ] += VOL_RELEASE_Constant [ Channel ];
					}
					else
					{
						//////////////////////////////////////
						// exponential decrease
						Temp = (u64) (VOL_ADSR_Value [ Channel ]);
						VOL_ADSR_Value [ Channel ] = (s64) ( ( Temp >> 15 ) * ( VOL_RELEASE_Constant [ Channel ] >> 15 ) );
						//VOL_ADSR_Value [ Channel ] = ( ( VOL_ADSR_Value [ Channel ] >> 16 ) * VOL_RELEASE_Constant [ Channel ] ) >> 15;
					}
					*/
...
static u32 SPU::Read ( u32 Address )
{
...
//#endif
	...
	debug << "; (before)VOL_L=" << _SPU->Regs [ ( ( Channel << 4 ) + VOL_L ) >> 1 ];
				...
				/*
				SetSweepVars ( Data, Channel, (s32*) _SPU->VOL_L_Constant, (s32*) _SPU->VOL_L_Constant75 );

				// if volume is constant, then just store
				if ( ! ( Data >> 15 ) )
				{
					// volume is constant //
					
					_SPU->VOL_L_Value [ Channel ] = ( (u32) Data ) << 17;
					
					// sign extend to 64-bits
					_SPU->VOL_L_Value [ Channel ] = ( _SPU->VOL_L_Value [ Channel ] << 32 ) >> 32;
				}
				*/
	...
	debug << "; (before)VOL_R=" << _SPU->Regs [ ( ( Channel << 4 ) + VOL_R ) >> 1 ];
				...
				/*
				SetSweepVars ( Data, Channel, (s32*) _SPU->VOL_R_Constant, (s32*) _SPU->VOL_R_Constant75 );
				...
				if ( ! ( Data >> 15 ) )
				{
					...
					_SPU->VOL_R_Value [ Channel ] = ( (u32) Data ) << 17;
					
					// sign extend to 64-bits
					_SPU->VOL_R_Value [ Channel ] = ( _SPU->VOL_R_Value [ Channel ] << 32 ) >> 32;
				}
				*/
	...
	debug << "; (before)ADSR_0=" << _SPU->Regs [ ( ( Channel << 4 ) + ADSR_0 ) >> 1 ];
				...
				// set decay rate
				Set_ExponentialDecRates ( Channel, ( ( Data >> 4 ) & 0xf ) << 2, (s32*) _SPU->VOL_DECAY_Constant, (s32*) _SPU->VOL_DECAY_Constant );
				...
				if ( Data >> 15 )
				{
					...
					Set_ExponentialIncRates ( Channel, ( Data >> 8 ) & 0x7f, (s32*) _SPU->VOL_ATTACK_Constant, (s32*) _SPU->VOL_ATTACK_Constant75 );
				}
				else
				{
					/////////////////////////////////////////////////////////////
					// linear increase
					Set_LinearIncRates ( Channel, ( Data >> 8 ) & 0x7f, (s32*) _SPU->VOL_ATTACK_Constant, (s32*) _SPU->VOL_ATTACK_Constant75 );
				}
	...
	debug << "; (before)ADSR_1=" << _SPU->Regs [ ( ( Channel << 4 ) + ADSR_1 ) >> 1 ];
				...
				if ( ( Data >> 5 ) & 1 )
				{
					/////////////////////////////////////////////////
					// exponential decrease
					Set_ExponentialDecRates ( Channel, ( Data & 0x1f ) << 2, (s32*) _SPU->VOL_RELEASE_Constant, (s32*) _SPU->VOL_RELEASE_Constant );
				}
				else
				{
					/////////////////////////////////////////////////
					// linear decrease
					Set_LinearDecRates ( Channel, ( Data & 0x1f ) << 2, (s32*) _SPU->VOL_RELEASE_Constant, (s32*) _SPU->VOL_RELEASE_Constant );
				}
				...
				switch ( ( Data >> 14 ) & 3 )
				{
					case 0:
						...
						//////////////////////////////////////////////////////////////////
						// linear increase
						Set_LinearIncRates ( Channel, ( Data >> 6 ) & 0x7f, (s32*) _SPU->VOL_SUSTAIN_Constant, (s32*) _SPU->VOL_SUSTAIN_Constant75 );
					...
					case 1:
						...
						////////////////////////////////////////////////////////////////////
						// linear decrease
						Set_LinearDecRates ( Channel, ( Data >> 6 ) & 0x7f, (s32*) _SPU->VOL_SUSTAIN_Constant, (s32*) _SPU->VOL_SUSTAIN_Constant75 );
					...
					case 2:
						...
						/////////////////////////////////////////////////////////////////////
						// pseudo exponential increase
						Set_ExponentialIncRates ( Channel, ( Data >> 6 ) & 0x7f, (s32*) _SPU->VOL_SUSTAIN_Constant, (s32*) _SPU->VOL_SUSTAIN_Constant75 );
					...
					case 3:
						...
						/////////////////////////////////////////////////////////
						// exponential decrease
						Set_ExponentialDecRates ( Channel, ( Data >> 6 ) & 0x7f, (s32*) _SPU->VOL_SUSTAIN_Constant, (s32*) _SPU->VOL_SUSTAIN_Constant75 );
	...
	debug << "; (before)MVOL_L=" << _SPU->Regs [ ( MVOL_L - SPU_X ) >> 1 ];
				...
				/*
				SetSweepVars ( Data, 0, (s32*) &_SPU->MVOL_L_Constant, (s32*) &_SPU->MVOL_L_Constant75 );
				...
				if ( ! ( Data >> 15 ) )
				{
					...
					_SPU->MVOL_L_Value = ( (u32) Data ) << 17;
					
					// sign extend to 64-bits
					_SPU->MVOL_L_Value = ( _SPU->MVOL_L_Value << 32 ) >> 32;
				}
				*/
	...
	debug << "; (before)MVOL_R=" << _SPU->Regs [ ( MVOL_R - SPU_X ) >> 1 ];
				...
				/*
				SetSweepVars ( Data, 0, (s32*) &_SPU->MVOL_R_Constant, (s32*) &_SPU->MVOL_R_Constant75 );
				...
				if ( ! ( Data >> 15 ) )
				{
					...
					_SPU->MVOL_R_Value = ( (u32) Data ) << 17;
					
					// sign extend to 64-bits
					_SPU->MVOL_R_Value = ( _SPU->MVOL_R_Value << 32 ) >> 32;
				}
				*/
...
bool SPU::DMA_ReadyForRead ( void )
{
	return true;
}


bool SPU::DMA_ReadyForWrite ( void )
{
	return true;
}


void SPU::DMA_Read ( u32* Data, int ByteReadCount )
{
	u32 Output;
...
void SPU::DMA_Write ( u32* Data, int BlockSizeInWords32 )
{
...
void SPU::DMA_Write_Block ( u32* Data, u32 BS )
{
#ifdef INLINE_DEBUG_DMA_WRITE
	debug << "\r\nDMA_Write: ";
	...
	for ( int i = 0; i < (BS << 1); i++ )
	{
		...
		RAM [ ( ( NextSoundBufferAddress + ( i << 1 ) ) & c_iRam_Mask ) >> 1 ] = ((u16*) Data) [ i ];
...
static void SPU::SetSweepVars ( u16 flags, u32 Channel, s32* Rates, s32* Rates75 )
{
	u32 Rate;
...
static void SPU::SweepVolume ( u16 flags, s64& CurrentVolume, u32 VolConstant, u32 VolConstant75 )
{
	u64 Temp;
...
static s64 SPU::Get_VolumeStep ( s64& Level, u32& Cycles, u32 Value, u32 flags )
{
...
static void SPU::Start_VolumeEnvelope ( s64& Level, u32& Cycles, u32 Value, u32 flags, bool InitLevel )
{
...
static void SPU::VolumeEnvelope ( s64& Level, u32& Cycles, u32 Value, u32 flags )
{
	//static const s32 StepValues_Inc [] = { 7, 6, 5, 4 };
	//static const s32 StepValues_Dec [] = { -8, -7, -6, -5 };
...
void SPU::ProcessReverbR ( s64 RightInput )
{
	s64 Rin, Rout;
...
void SPU::ProcessReverbL ( s64 LeftInput )
{
	s64 Lin, Lout;
...
s64 SPU::ReadReverbBuffer ( u32 Address )
{
...
void SPU::WriteReverbBuffer ( u32 Address, s64 Value )
{
...
static s32 SPU::Calc_sample_gx ( u32 SampleOffset_Fixed16, s32 Sample0, s32 Sample1, s32 Sample2, s32 Sample3 )
{
	u32 i;
	s32 Output;
...
static s32 SPU::Calc_sample_filter ( s32 x0, s32 x1, s32 x2, s32 y1, s32 y2 )
{
	s32 Output;
...
void SPU::InvalidateCachedSampleBlock ( u32 Address )
{
}
...
u32 SPU::GetSampleFromBlock ( u32 BlockStartAddress, u32 SampleNumberInBlock )
{
}

// need to be able to input CD-DA data from CD device
// should receive raw samples from CD
void SPU::CDTransfer_CDDA ( u32* Data, u32 SizeInBytes )
{
}

// need to be able to input CD-XA data from CD device
// should receive raw samples from CD
void SPU::CDTransfer_CDXA ( u32* Data, u32 SizeInBytes )
{
}
*/
...
static void SPU::DebugWindow_Enable ()
{
...
static void SPU::DebugWindow_Disable ()
{
...
static void SPU::DebugWindow_Update ()
{
