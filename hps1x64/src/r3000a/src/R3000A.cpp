...
void Cpu::Run ()
{
	...
	//{
		...
		{
			// bus is definitely busy and instruction pipeline is stalled //
	...
	// clear this here for now
	//LastModifiedRegister = 256;
	...
	// if the instruction was not executed successfully, then just run the required cpu events and return
	/*
	if ( !CurInstExecuted )
	{
#ifdef INLINE_DEBUG
	...
	*/
	...
	if ( Status.isSomethingBusy )
	{
		...
		if ( /*Status.AsynchronousInterrupt*/ Status.isExternalInterrupt /*&& CPR0.Status.IEc*/ )
		{
			if ( CPR0.Status.IEc )
			{
				///////////////////////
			...
			// here we have a GTE opcode next to execute, so we want to update delay slots but keep interrupt
			Status.DelaySlot_Valid = ( Status.DelaySlot_Valid & 0xfc ) | ( ( Status.DelaySlot_Valid << 1 ) & 0x3 );
			*/
		}
		/*
		else
		{
			///////////////////////////////////////////////////
			// Advance status bits for checking delay slot
			//Status.DelaySlot_Valid = ( Status.DelaySlot_Valid << 1 ) & 0x73;
			//Status.DelaySlot_Valid = ( Status.DelaySlot_Valid << 1 ) & 0x3;
...
void Cpu::ProcessRequiredCPUEvents ()
{
	...
	if ( Bus->isReady () )
	{
		...
		{
			/*
			if ( LoadBuffer.isNextLoad () )
			{
				ProcessExternalLoad ();
				LoadBuffer.InvalidateLoad ();
				
				/////////////////////////////////////////////////////////////////////////
				// Only want to advance in buffer if we process load successfully
				LoadBuffer.Advance ();
			}
			else
			{
...
u32 ExitCriticalSection_Test = 0;
...
static void Cpu::DebugWindow_Enable ()
{
	...
	if ( !DebugWindow_Enabled )
	{
		...
		for ( i = 0; i < 32; i++ )
		{
			ss.str ("");
			ss << "R" << dec << i;
			GPR_ValueList->AddVariable ( ss.str().c_str(), &(_CPU->GPR [ i ].u) );
			...
			COP0_ValueList->AddVariable ( ss.str().c_str(), &(_CPU->CPR0.Regs [ i ]) );
		...
		*/
		...
		DisAsm_Window->Create ( DebugWindow, DisAsm_X, DisAsm_Y, DisAsm_Width, DisAsm_Height, Print::PrintInstruction );
...
static void Cpu::DebugWindow_Disable ()
{
...
static void Cpu::DebugWindow_Update ()
{
