...
static const u32 CommandExecutionTimes [] = { 0x900 /*1*/, 30547, 31557, 51444, 193250, 125119, 75701, 74753,
...
u32 CD::DMA_ReadBlock ( u32* Data, u32 BS, u32 BA )
{
	...
	lsn = DVDSectorNumber + 0x30000;
	
	PerformXor = DecryptSetting & 1;
	PerformShift = DecryptSetting & 2;
	ShiftAmount = ( DecryptSetting >> 4 ) & 7;
#endif
	...
	*/
	...
	{
		// need to account for transferring more than one block
		WordTransferCount = BS * BA;
	}
	
#ifdef PS2_COMPILE
	// for now, also take the minimum of WordTransferCount and DataBuffer_Size
	WordTransferCount = ( WordTransferCount <= ( DataBuffer_Size >> 2 ) ) ? WordTransferCount : ( DataBuffer_Size >> 2 );
#endif
	...
	for ( int i = 0; i < WordTransferCount; i++ )
	{
		...
		if ( ReadMode == 1024 )
		{
			if ( !i )
			{
				memset ( (u8*) Data, 0, DataBuffer_Size );
		...
		else
		{
			...
			if ( ReadMode == 2064 )
			{
				Data [ i ] = ((u32*)pSectorDataBuffer) [ ( ReadMode_Offset + ( DataBuffer_Index - 12 ) ) >> 2 ];
			}
			else
			{
#endif
