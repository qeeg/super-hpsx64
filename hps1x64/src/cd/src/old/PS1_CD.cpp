...
static const u32 CommandExecutionTimes [] = { 1, 30547, 31557, 51444, 193250, 125119, 75701, 74753,
...
static const u32 CommandExecutionTimes_Secondary [] = { 0, 0, 0, 0, 0, 0, 450666, 51429486,
...
static const char* CommandList [] = { "Sync", "Nop", "SetLoc", "Play", "Forward", "Backward", "ReadN", "Standby",
...
#ifdef INLINE_DEBUG_RUN || INLINE_DEBUG_PLAY
...
#ifdef INLINE_DEBUG_RUN
	debug << " CurrentSector_Number (CD)=" << dec << cd_image.CurrentSector << " CurrentSector_Number (Local)=" CurrentSector_Number << " NextTrackSector_Number=" << NextTrackSector_Number;
#endif
...
//#endif
	...
	debug << "; Param=" << dec << (u32)Get_Parameter ( 0 ) << " Track=" << (u32)Track << " AMin=" << dec << (u32)AMin << " ASec=" << (u32)ASec << " AFrac=" << (u32)AFrac;
					...
					if ( ( NextTrackSector_Number - CurrentTrack_Number ) < 150 )
					{
						// try again
						NextTrack_Number++;
						cd_image.GetTrackStart ( NextTrack_Number, Next_AMin, Next_ASec, Next_AFrac );
...
static int CD::Get_MaxChan ( u8 CodingInfo, int DoubleSpeed )
{
...
void CD::Process_Read ()
{
	...
	*/
	...
	debug << "; SIMSUBQ; Track=" << (u32)cd_image.SubQ_Track;
	debug << "; Index=" << (u32)cd_image.SubQ_Index;
	...
	debug << "; IntQueue.Size=" << dec << IntQueue.Size ();
					...
					if ( ReadMode == 2048 )
					{
						...
						SectorDataQ_BufferSize [ SectorDataQ_Index & c_iMaxQueuedSectors_Mask ] = 2048;
						SectorDataQ_BufferOffset [ SectorDataQ_Index & c_iMaxQueuedSectors_Mask ] = 24;
					}
					else
					{
						SectorDataQ_Buffer [ SectorDataQ_Index & c_iMaxQueuedSectors_Mask ] = cd_image.GetCurrentBufferIndex ();
						...
						SectorDataQ_BufferSize [ SectorDataQ_Index & c_iMaxQueuedSectors_Mask ] = 2340;
						SectorDataQ_BufferOffset [ SectorDataQ_Index & c_iMaxQueuedSectors_Mask ] = 12;
					}
...
if ( _CD->DebugCount > 0 )
{
	debug << "; REMOVE";
		...
		// before removing an interrupt, clear out its data buffer
		/*
		if ( IntQueue.Peek ()->isDataReadyInterrupt )
		{
			// deactivate buffer
			_CD->SectorDataQ_Active [ IntQueue.Peek ()->SectorDataQ_Index & c_iMaxQueuedSectors_Mask ] = 0;
		}
		*/
...
if ( _CD->DebugCount > 0 )
{
	debug << "; INT_PendingRead";
		...
		// go to next interrupt - only if it is enabled
		/*
		if ( InterruptQ [ ( InterruptQ_Index + 1 ) & c_InterruptQ_Mask ].InterruptEnabled )
		{
			InterruptQ_Index = ( InterruptQ_Index + 1 ) & c_InterruptQ_Mask;
		}

		// update the interrupt flag again
		REG_InterruptFlag = ( REG_InterruptFlag & 0xf0 ) | ( InterruptQ [ InterruptQ_Index ].InterruptReason & 0xf );
		
		// trigger interrupt if it exists
		if ( InterruptQ [ InterruptQ_Index ].InterruptEnabled && ( REG_InterruptFlag & 0x1f ) )
		{
			// trigger interrupt
			SetInterrupt ();
		...
		{
			// interrupt is acknowledged and there are no more interrupts
			ClearInterrupt ();
		}
		*/
	}
...
static u32 CD::Read ( u32 Address )
{
	u32 Data;
	u8* pSectorDataBuffer;
...
static void CD::Write ( u32 Address, u32 Data, u32 Mask )
{
...
if ( _CD->DebugCount > 0 )
{
...
#ifdef INLINE_DEBUG_RUN
	debug << "\r\n\r\nTESTING: DataBuffer_SlotNumber=" << dec << _CD->DataBuffer_SlotNumber << " DataBuffer_Index=" << _CD->DataBuffer_Index << " DataBuffer_Size=" << _CD->DataBuffer_Size << " ReadCount=" << _CD->ReadCount;
	debug << " Active=" << dec << _CD->SectorDataQ_Active [ _CD->DataBuffer_SlotNumber & c_iMaxQueuedSectors_Mask ] << " Active+1=" << _CD->SectorDataQ_Active [ ( _CD->DataBuffer_SlotNumber + 1 ) & c_iMaxQueuedSectors_Mask ];
	debug << " SectorDataQ_Index=" << dec << _CD->SectorDataQ_Index;
	debug << " LastDataBufferIndex=" << _CD->LastDataBufferIndex;
#endif
						...
						// keep updating incase multiple sectors were skipped
						//for ( int i = 0; i < c_iMaxQueuedSectors; i++ )
						//{
						
							// update data buffer slot number
							
							if ( ( !_CD->SectorDataQ_Active [ _CD->DataBuffer_SlotNumber & c_iMaxQueuedSectors_Mask ] ) && _CD->DataBuffer_SlotNumber < _CD->SectorDataQ_Index )
							{
						...
						//_CD->DataBuffer_SlotNumber = _CD->LastDataBufferIndex;
						//_CD->SectorDataBuffer_Index = _CD->SectorDataQ_Buffer [ _CD->LastDataBufferIndex & c_iMaxQueuedSectors_Mask ];
						
						/*
						if ( _CD->DataBuffer_SlotNumber != _CD->LastDataBufferIndex )
						{
							cout << "\nDataBuffer_SlotNumber != _CD->LastDataBufferIndex\n";
							debug << "\r\nDataBuffer_SlotNumber != _CD->LastDataBufferIndex\r\n";
						}
						*/
	...
	debug << " DataBuffer_SlotNumber=" << dec << _CD->DataBuffer_SlotNumber << " DataBuffer_Index=" << _CD->DataBuffer_Index << " DataBuffer_Size=" << _CD->DataBuffer_Size << " ReadCount=" << _CD->ReadCount;
	debug << " SectorDataQ_Index=" << dec << _CD->SectorDataQ_Index;
	debug << " LastDataBufferIndex=" << _CD->LastDataBufferIndex;
#endif
					}
	...
	debug << " DataBuffer_SlotNumber=" << dec << _CD->DataBuffer_SlotNumber;
#endif
...
if ( _CD->DebugCount > 0 )
{
					...
					*/
					...
					_CD->REG_InterruptFlag &= ~( Data /*& _CD->REG_InterruptEnable*/ );
					...
					*/
					...
					if ( _CD->IntQueue.Peek () ) _CD->IntQueue.Peek()->InterruptReason &= ~( Data /*& _CD->REG_InterruptEnable*/ );
...
u32 CD::DMA_ReadBlock ( u32* Data, u32 BS, u32 BA )
{
#ifdef INLINE_DEBUG_RUN
	debug << "\r\nDMA_ReadBlock" << dec;
	debug << "; BS=" << BS;
	debug << "; BA=" << BA;
	debug << "; DataBuffer_Index=" << DataBuffer_Index;
	debug << "; DataBuffer_Size=" << DataBuffer_Size;
#endif
	...
	*/
	...
	for ( int i = 0; i < BS; i++ )
	{
		...
		//{
			...
			Data [ i ] = ((u32*)pSectorDataBuffer) [ ( ReadMode_Offset + DataBuffer_Index ) >> 2 ];
...
static int CD::isDataSector ( unsigned char* data )
{
...
//#endif
	...
	if ( IntQueue.Peek () )
	{
		...
		// is not a data ready interrupt
		IntQ_Entry->isDataReadyInterrupt = 0;
...
void CD::EnqueueInterrupt_Read ( u8* Data, u32 Size, u8 Interrupt )
{
	...
	debug << "\r\nCD::EnqueueInterrupt_Read; Size=" << Size << " Interrupt=" << (u32) Interrupt << " QueueSize=" << IntQueue.Size();
#endif
	...
	if ( pIntQ_Entry )
	{
		...
		pIntQ_Entry->isDataReadyInterrupt = 0;
		if ( Interrupt == 1 )
		{
			pIntQ_Entry->isDataReadyInterrupt = 1;
		}
...
{
	if ( NextRead_Cycle > *_DebugCycleCount && ( NextRead_Cycle < NextAction_Cycle || NextAction_Cycle <= *_DebugCycleCount ) && ( NextRead_Cycle < NextStart_Cycle || NextStart_Cycle <= *_DebugCycleCount ) )
	{
		// the next read event is the next event for device
		NextEvent_Cycle = NextRead_Cycle;
	}
	else if ( NextAction_Cycle > *_DebugCycleCount && ( NextAction_Cycle < NextStart_Cycle || NextStart_Cycle <= *_DebugCycleCount ) )
	{
		NextEvent_Cycle = NextAction_Cycle;
	}
	else if ( NextStart_Cycle > *_DebugCycleCount )
	{
...
static u8 CD::ConvertBCDToDec ( u8 BCD )
{
	if ( BCD > 0x99 ) BCD -= 0x80;
	return ( BCD & 0xf ) + ( ( BCD >> 4 ) * 10 );
}

static u8 CD::ConvertDecToBCD ( u8 Dec )
{
	return ( ( Dec / 10 ) << 4 ) + ( ( Dec ) % 10 );
}


static void CD::Inc_DiskPosition ( u8& tMin, u8& tSec, u8& tFrac )
{
	tFrac++;
	if ( tFrac >= 75 )
	{
...
void CD::Process_XASector ( u8* pSectorDataBuffer )
{
	...
	SampleRateAndChannels = DecodeXA_Sector ( TempXA_Buffer, pSectorDataBuffer );
...
static void CD::DebugWindow_Enable ()
{

#ifndef _CONSOLE_DEBUG_ONLY_

	static const char* DebugWindow_Caption = "PS1 CD Debug Window";
	...
	if ( !DebugWindow_Enabled )
	{
		...
		ValueList->AddVariable ( "MSTAT", & _CD->REG_ModeStatus );
		ValueList->AddVariable ( "IENABLE", & _CD->REG_InterruptEnable );
		ValueList->AddVariable ( "IFLAG", & _CD->REG_InterruptFlag );
...
static void CD::DebugWindow_Disable ()
{
...
static void CD::DebugWindow_Update ()
{
