...
public:
	...
	bool AddVariable ( char* Name, Type* Var );
	
	bool ModifyVariable ( int index, Type NewValue );
	
	Type GetVariable ( int index ) { return *(ListOfVariables [ index ]); }
	
	bool Update ();
	
	inline bool EnableVariableEdits () { Locked = false; }
	inline bool DisableVariableEdits () { Locked = true; }
...
// this will look for the variable in all the value lists created for Type
template<typename Type>
static Type* DebugValueList<Type>::FindVariableGlobal ( string Name )
{
	int i;
	Type* Var;
...
template<typename Type>
bool DebugValueList<Type>::ModifyVariable ( int index, Type NewValue )
{
...
template<typename Type>
bool DebugValueList<Type>::AddVariable ( char* Name, Type* Var )
{
	// insert row
...
template<typename Type>
static void DebugValueList<Type>::DebugValueList_DblClick ( HWND hCtrl, int idCtrl, unsigned int message, WPARAM wParam, LPARAM lParam )
{
	int i;
...
template<typename Type>
static void DebugValueList<Type>::Dialog1_OkClick ( HWND hCtrl, int idCtrl, unsigned int message, WPARAM wParam, LPARAM lParam )
{
	int i;
	Type Value;
	HWND Parent_hWnd;
...
template<typename Type>
static void DebugValueList<Type>::Dialog1_CancelClick ( HWND hCtrl, int idCtrl, unsigned int message, WPARAM wParam, LPARAM lParam )
{
	int i;
	HWND Parent_hWnd;
