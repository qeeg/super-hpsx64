...
bool File::Create ( string fileName, unsigned long Creation = CREATE_ALWAYS, unsigned long Access = GENERIC_WRITE, unsigned long Attributes = FILE_ATTRIBUTE_NORMAL | FILE_FLAG_WRITE_THROUGH )
{
...
bool File::CreateSync ( string fileName, unsigned long Creation = OPEN_EXISTING, unsigned long Access = GENERIC_READ, unsigned long Attributes = FILE_ATTRIBUTE_NORMAL )
{
	hFile = CreateFile ( fileName.c_str (), Access, NULL, NULL, Creation, Attributes, NULL );
	if ( hFile == INVALID_HANDLE_VALUE ) return false;
	return true;
...
bool File::CreateAsync ( string fileName, unsigned long Creation = OPEN_EXISTING, unsigned long Access = GENERIC_READ, unsigned long Attributes = FILE_ATTRIBUTE_NORMAL | FILE_FLAG_OVERLAPPED )
{
	hFile = CreateFile ( fileName.c_str (), Access, NULL, NULL, Creation, Attributes, NULL );
	if ( hFile == INVALID_HANDLE_VALUE ) return false;
	return true;
