...
LRESULT CALLBACK WindowClass::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	for ( i = 0; i < WindowClass::Window::EventList.size (); i++ )
	{
		...
		if ( ( e->hwndParent == hWnd ) && ( e->message == message || e->message == NULL ) && ( e->id == LOWORD(wParam) || e->id == NULL ) )
		{
...
{
	PostMessage ( hWnd, GUI_CHANGETEXTCOLOR, (WPARAM) color, (LPARAM) NULL );
...
{
	PostMessage ( hWnd, GUI_CHANGEBKMODE, (WPARAM) iBkMode, (LPARAM) NULL );
...
static void WindowClass::Window::_DrawPixel ( HWND _hWnd, int x, int y, COLORREF ColorRGB24 )
{
	HDC _hDC;
...
bool WindowClass::Window::SetText ( long x, long y, char* text, u32 fontSize, UINT uFormat )
{
	_wParams p( x, y, fontSize, uFormat );
	PostMessage ( hWnd, GUI_SETTEXT, (WPARAM) p.Value, (LPARAM) new string( text ) );
}

// this calls the function from the GUI thread
static bool WindowClass::Window::_SetText ( HWND _hWnd, long x, long y, char* text, u32 fontSize, UINT uFormat )
{
	RECT rect;
	HDC _hDC;
...
long WindowClass::Window::_GetTextHeight ( HWND _hWnd, HDC _hDC, char* text, UINT uFormat )
{
	RECT rect;
...
long WindowClass::Window::_GetTextWidth ( HWND _hWnd, HDC _hDC, char* text, UINT uFormat )
{
	RECT rect;
...
bool WindowClass::Window::PrintText ( long x, long LineNumber, char* text, u32 fontSize, UINT uFormat )
{
	_wParams p( x, LineNumber, fontSize, uFormat );
	PostMessage ( hWnd, GUI_PRINTTEXT, (WPARAM) p.Value, (LPARAM) new string( text ) );
}

bool WindowClass::Window::_PrintText ( HWND _hWnd, long x, long LineNumber, char* text, u32 fontSize, UINT uFormat )
{
	long LineHeight;
	RECT rect;
	HDC _hDC;
...
{
	PostMessage ( hWnd, GUI_CLEAR, (WPARAM) NULL, (LPARAM) NULL );
	/*
	RECT rect;
...
static WindowClass::MenuBar* WindowClass::MenuBar::GetMenuBarForWindow ( HWND _hWnd )
{
...
static WindowClass::MenuBar* WindowClass::MenuBar::FindMenuBarById ( u32 _Id )
{
	for ( int i = 0; i < ListOfMenuBars.size(); i++ )
	{
		// if this menu bar is found, then return it
...
static bool WindowClass::Window::AddEvent ( HWND hParentWindow, HWND hControl, int CtrlId, unsigned int message, EventFunction Func )
{
...
static bool WindowClass::Window::RemoveEvent ( HWND hParentWindow, int id, unsigned int message )
{
	bool event_removed;
	...
	do
	{
		...
		{
			// if this menu bar is found, then delete it
			if ( ( (*i)->id == id || id == NULL ) && ( (*i)->message == message || message == NULL ) && (*i)->hwndParent == hParentWindow )
			{
				// object is already being deleted, so don't delete or it crashes!
...
static unsigned long long WindowClass::Window::RemoteCall ( WindowClass::Window::RemoteFunction FunctionToCall, void* Parameters, bool WaitForReturnValue )
{
	int i;
...
HWND WindowClass::Window::CreateControl ( char* ClassName, int x, int y, int width, int height, char* Caption, int flags, HMENU hMenu )
{
	HWND handle;

	_CreateControl_Params* Params = new _CreateControl_Params ( hWnd, ClassName, x, y, width, height, Caption, flags, hMenu );
	handle =  (HWND) RemoteCall ( (Function) _CreateControl, (void*) Params );
...
static unsigned long long WindowClass::Window::_CreateControl ( _CreateControl_Params* p )
{
#ifdef INLINE_DEBUG
...
HWND WindowClass::Window::Create ( char* _lpWindowName, int _x, int _y, int _nWidth, int _nHeight, DWORD _dwStyle, HMENU _hMenu, HWND _hWndParent, LPVOID _lpParam, HINSTANCE _hInstance, LPCTSTR _lpClassName )
{
	...
	hWnd = (HWND) RemoteCall ( (Function) _Create, (void*) Params );
...
static void WindowClass::Window::OutputAllDisplayModes ()
{
DEVMODE dm = { 0 };
dm.dmSize = sizeof(dm);
...
bool WindowClass::Window::Redraw ()
{
	...
	// call the function on gui thread and get the handle to the window that was created
	b = (bool) RemoteCall ( (Function) _Redraw, (void*) p );
	
	delete p;
	return b;
}

static unsigned long long WindowClass::Window::_Redraw ( _Redraw_Params* p )
{
#ifdef INLINE_DEBUG
	debug << "\r\nRemoteCall: _Redraw";
#endif
...
bool WindowClass::Window::Destroy ()
{
	...
	// call the function on gui thread and get the handle to the window that was created
	b = (bool) RemoteCall ( (Function) _Destroy, (void*) p );
...
static unsigned long long WindowClass::Window::_Destroy ( _Destroy_Params* p )
{
#ifdef INLINE_DEBUG
	debug << "\r\nRemoteCall: _Destroy";
#endif
...
volatile int WindowClass::Window::Show_ContextMenu ( int x, int y, string ContextMenu_Options )
{
	...
	ReturnValue =  (int) RemoteCall ( (Function) _Show_ContextMenu, (void*) Params );
...
static unsigned long long WindowClass::Window::_Show_ContextMenu ( _Show_ContextMenu_Params* p )
{
...
static bool WindowClass::Window::GetRequiredWindowSize ( long* width, long* height, BOOL hasMenu, long WindowStyle )
{
	RECT Rect;
	bool ret;
...
HWND WindowClass::Button::Create_CmdButton ( WindowClass::Window* ParentWindow, int x, int y, int width, int height, char* Caption, int Id, int flags )
{
	Parent = ParentWindow;
	id = Id;
...
HWND WindowClass::Button::Create_CheckBox ( WindowClass::Window* ParentWindow, int x, int y, int width, int height, char* Caption, int Id, int flags )
{
	Parent = ParentWindow;
	id = Id;
	hWnd = ParentWindow->CreateControl ( ClassName, x, y, width, height, Caption, flags, (HMENU) id );
...
HWND WindowClass::Button::Create_RadioButtonGroup ( WindowClass::Window* ParentWindow, int x, int y, int width, int height, char* Caption, int Id, int flags )
{
	Parent = ParentWindow;
	id = Id;
	hWnd = ParentWindow->CreateControl ( ClassName, x, y, width, height, Caption, flags, (HMENU) id );
...
HWND WindowClass::Button::Create_RadioButton ( WindowClass::Window* ParentWindow, int x, int y, int width, int height, char* Caption, int Id, int flags )
{
	Parent = ParentWindow;
	id = Id;
	hWnd = ParentWindow->CreateControl ( ClassName, x, y, width, height, Caption, flags, (HMENU) id );
...
HWND WindowClass::Edit::Create ( WindowClass::Window* ParentWindow, int x, int y, int width, int height, char* Caption, int Id, int flags )
{
	Parent = ParentWindow;
	id = Id;
	hWnd = ParentWindow->CreateControl ( ClassName, x, y, width, height, Caption, flags, (HMENU) id );
...
HWND WindowClass::ComboBox::Create_Simple ( Window* ParentWindow, int x, int y, int width, int height, char* Caption, int Id, int flags )
{
	Parent = ParentWindow;
	id = Id;
	hWnd = ParentWindow->CreateControl ( ClassName, x, y, width, height, Caption, flags, (HMENU) id );
...
HWND WindowClass::ComboBox::Create_DropDown ( Window* ParentWindow, int x, int y, int width, int height, char* Caption, int Id, int flags )
{
	Parent = ParentWindow;
	id = Id;
	hWnd = ParentWindow->CreateControl ( ClassName, x, y, width, height, Caption, flags, (HMENU) id );
...
HWND WindowClass::ComboBox::Create_DropDownList ( Window* ParentWindow, int x, int y, int width, int height, char* Caption, int Id, int flags )
{
	Parent = ParentWindow;
	id = Id;
	hWnd = ParentWindow->CreateControl ( ClassName, x, y, width, height, Caption, flags, (HMENU) id );
...
HWND WindowClass::Static::Create_Text ( Window* ParentWindow, int x, int y, int width, int height, char* Caption, int Id, int flags )
{
	Parent = ParentWindow;
	id = Id;
	hWnd = ParentWindow->CreateControl ( ClassName, x, y, width, height, Caption, flags, (HMENU) id );
...
HWND WindowClass::Static::Create_Bitmap ( Window* ParentWindow, int x, int y, int width, int height, char* Caption, int Id, int flags )
{
	Parent = ParentWindow;
	id = Id;
	hWnd = ParentWindow->CreateControl ( ClassName, x, y, width, height, Caption, flags, (HMENU) id );
...
HWND WindowClass::ListView::Create_wHeader ( Window* ParentWindow, int x, int y, int width, int height, char* Caption, int Id, int flags )
{
	Parent = ParentWindow;
	id = Id;
...
HWND WindowClass::ListView::Create_NoHeader ( Window* ParentWindow, int x, int y, int width, int height, char* Caption, int Id, int flags )
{
	Parent = ParentWindow;
	id = Id;
	hWnd = ParentWindow->CreateControl ( WC_LISTVIEW, x, y, width, height, Caption, flags, (HMENU) id );
...
HWND WindowClass::ListView::Create_Dynamic_wHeader ( Window* ParentWindow, int x, int y, int width, int height, char* Caption, int Id, int flags )
{
	Parent = ParentWindow;
	id = Id;
	hWnd = ParentWindow->CreateControl ( WC_LISTVIEW, x, y, width, height, Caption, flags, (HMENU) id );
...
static HFONT WindowClass::Window::CreateFontObject ( int fontSize, char* fontName, bool Bold, bool Underline, bool Italic, bool StrikeOut )
{
	HFONT hF;
	
	//PostMessage ( hWnd, GUI_CHANGEFONT, (WPARAM) fontSize, (LPARAM) new string ( fontName ) );
	_CreateFontObject_Params *p = new _CreateFontObject_Params ( fontSize, fontName, Bold, Underline, Italic, StrikeOut );
	hF = (HFONT) RemoteCall ( _CreateFontObject, (void*) p );
...
int WindowClass::Window::Get_TextWidth ( string text, UINT uFormat )
{
	...
	ReturnValue = (int) RemoteCall ( _Get_TextWidth, (void*) p );
...
int WindowClass::Window::Get_TextHeight ( string text, UINT uFormat )
{
	...
	_Get_TextWidth_Params *p = new _Get_TextWidth_Params ( hWnd, text, uFormat );
	ReturnValue = (int) RemoteCall ( _Get_TextHeight, (void*) p );
...
static unsigned long long WindowClass::Window::_Get_TextWidth ( _Get_TextWidth_Params* p )
{
	RECT rect;
	HDC _hDC;
...
static unsigned long long WindowClass::Window::_Get_TextHeight ( _Get_TextWidth_Params* p )
{
/*
	RECT rect;
...
void WindowClass::Window::Set_Font ( HFONT _hFont )
{
	...
	ReturnValue = (int) RemoteCall ( _Set_Font, (void*) p );
	
	delete p;
}


static unsigned long long WindowClass::Window::_Set_Font ( _Set_Font_Params* p )
{
	HDC _hDC;
