...
namespace WindowClass
{
	...
	public:
		...
		HWND CreateControl ( char* ClassName, int x, int y, int width, int height, char* Caption, int flags, HMENU hMenu = NULL );
	
		HWND Create ( char* _lpWindowName, int _x, int _y, int _nWidth, int _nHeight, DWORD _dwStyle = DefaultStyle, HMENU _hMenu = NULL, HWND _hWndParent = NULL, LPVOID _lpParam = NULL, HINSTANCE _hInstance = NULL, LPCTSTR _lpClassName = className );
		...
		bool DisableCloseButton () { EnableMenuItem( GetSystemMenu( hWnd, false ), SC_CLOSE, MF_BYCOMMAND | MF_GRAYED ); }
...
*/
		...
		bool SetText ( long x, long y, char* text, u32 fontSize = 8, UINT uFormat = DT_SINGLELINE | DT_NOCLIP );

		// Draws text on a particular line number of the window
		bool PrintText ( long x, long LineNumber, char* text, u32 fontSize = 8, UINT uFormat = DT_SINGLELINE | DT_NOCLIP );
	...
	private:
		...
		struct _CreateControl_Params { HWND ParentWindow; char* ClassName; int x; int y; int width; int height; char* Caption; int flags; HMENU hMenu;
		_CreateControl_Params ( HWND _ParentWindow, char* _ClassName, int _x, int _y, int _width, int _height, char* _Caption, int _flags, HMENU _hMenu )
		...
		//////////// Create ///////////////////////
		// creates a window
		struct _Create_Params { char* lpWindowName; int x; int y; int nWidth; int nHeight; DWORD dwStyle; HMENU hMenu; HWND hWndParent; LPVOID lpParam;
								HINSTANCE hInstance; LPCTSTR lpClassName; WindowClass::Window* w;	// this is needed to set hWnd
		_Create_Params ( WindowClass::Window* _w, char* _lpWindowName, int _x, int _y, int _nWidth, int _nHeight, DWORD _dwStyle, HMENU _hMenu, HWND _hWndParent, LPVOID _lpParam, HINSTANCE _hInstance, LPCTSTR _lpClassName )
		...
		static bool _SetText ( HWND _hWnd, long _x, long _y, char* _text, u32 _fontSize, UINT _uFormat );
		...
		static bool _PrintText ( HWND _hWnd, long x, long LineNumber, char* text, u32 fontSize, UINT uFormat );

		// these can only be called from GUI thread with HDC
		static long _GetTextHeight ( HWND _hWnd, HDC _hDC, char* text, UINT uFormat = DT_CALCRECT );
		static long _GetTextWidth ( HWND _hWnd, HDC _hDC, char* text, UINT uFormat = DT_CALCRECT );
	...
	public:
		...
		HWND Create_CmdButton ( Window* ParentWindow, int x, int y, int width, int height, char* Caption = NULL, int id = NULL, int flags = DefaultFlags_CmdButton );
		...
		HWND Create_RadioButtonGroup ( Window* ParentWindow, int x, int y, int width = NULL, int height = NULL, char* Caption = NULL, int id = NULL, int flags = DefaultFlags_RadioButtonGroup );
		HWND Create_RadioButton ( Window* ParentWindow, int x, int y, int width = NULL, int height = NULL, char* Caption = NULL, int id = NULL, int flags = DefaultFlags_RadioButton );
		
		// this will create a check box on the window
		HWND Create_CheckBox ( Window* ParentWindow, int x, int y, int width = NULL, int height = NULL, char* Caption = NULL, int id = NULL, int flags = DefaultFlags_CheckBox );
		...
		inline bool Set_XY ( int x, int y ) { SetWindowPos ( hWnd, NULL, x, y, NULL, NULL, SWP_NOSIZE | SWP_NOZORDER ); }
		inline bool Set_Size ( int width, int height ) { SetWindowPos ( hWnd, NULL, NULL, NULL, width, height, SWP_NOMOVE | SWP_NOZORDER ); }
		...
		// this will add an event for a window/control
		// specify NULL if an argument doesn't matter
		inline bool AddEvent ( unsigned int message, WindowClass::Window::EventFunction Func ) { WindowClass::Window::AddEvent ( Parent->hWnd, hWnd, id, message, Func ); }
	...
	public:
		...
		HWND Create_Text ( Window* ParentWindow, int x, int y, int width, int height, char* Caption = NULL, int id = NULL, int flags = DefaultFlags_Text );
		HWND Create_Bitmap ( Window* ParentWindow, int x, int y, int width, int height, char* Caption = NULL, int id = NULL, int flags = DefaultFlags_Bitmap );
	...
	public:
		...
		HWND Create ( Window* ParentWindow, int x, int y, int width, int height, char* Caption = NULL, int id = NULL, int flags = DefaultFlags );
	...
	public:
		...
		HWND Create_Simple ( Window* ParentWindow, int x, int y, int width, int height, char* Caption = NULL, int id = NULL, int flags = DefaultFlags_Simple );
		HWND Create_DropDown ( Window* ParentWindow, int x, int y, int width, int height, char* Caption = NULL, int id = NULL, int flags = DefaultFlags_DropDown );
		HWND Create_DropDownList ( Window* ParentWindow, int x, int y, int width, int height, char* Caption = NULL, int id = NULL, int flags = DefaultFlags_DropDownList );
	...
	class ListBox
	{
		...
		inline int AddString ( char* text ) { SendDlgItemMessage( Parent->hWnd, id, LB_ADDSTRING, (WPARAM) NULL, (LPARAM) text ); }
		inline int DeleteString ( int index ) { SendDlgItemMessage( Parent->hWnd, id, LB_DELETESTRING, (WPARAM) index, (LPARAM) NULL ); }
	...
	public:
		...
		HWND Create_wHeader ( Window* ParentWindow, int x, int y, int width, int height, char* Caption = "", int id = NULL, int flags = DefaultFlags_Report_wHeader );
		HWND Create_NoHeader ( Window* ParentWindow, int x, int y, int width, int height, char* Caption = "", int id = NULL, int flags = DefaultFlags_Report_NoHeader );
		HWND Create_Dynamic_wHeader ( Window* ParentWindow, int x, int y, int width, int height, char* Caption = "", int id = NULL, int flags = DefaultFlags_Report_Dynamic_wHeader );
		...
		inline void SetItemText ( int row, int column, char* text ) { ListView_SetItemText( hWnd, row, column, (LPCTSTR) text ); }
